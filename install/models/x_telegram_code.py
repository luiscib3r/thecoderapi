...
def install_x_telegram_code():
    x_telegram_code_model = Model(
        name="Telegram Code",
        model="x_telegram_code",
    )

    model_id = crud_create_model(odoo, x_telegram_code_model)
    ...
    for field in x_telegram_code_fields:
        crud_create_model_field(odoo, field)

    x_telegram_code_access = ModelAccess(
        model_id=model_id,
        name="x_telegram_code_access"
    )

    crud_create_model_access(odoo, x_telegram_code_access)


def uninstall_x_telegram_code():
    model_id = crud_search_model(odoo, [["model", "=", "x_telegram_code"]])[0]
